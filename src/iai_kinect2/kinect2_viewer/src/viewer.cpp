...
private:
                ...
                const sensor_msgs::CameraInfo::ConstPtr cameraInfoColor, const sensor_msgs::CameraInfo::ConstPtr cameraInfoDepth)
  {
    ...
    //我绝了，为啥这步imshow会导致图像停止
    // cv::imshow("imagdfdf",color);
    // cv::waitKey(1);
  ...
  void imageViewer()
  {
    ...
    for (; running && ros::ok();)
    {
      if (updateImage)
      {
        ...
        //elapsed单位是秒，因为是毫秒/1000
        if (elapsed >= 1.0)
        {
          cout << "elapsed:" << elapsed << "  frameCount:" << frameCount << endl;
...
#pragma omp parallel for
  ...
  void createCloud(const cv::Mat &depth, const cv::Mat &color, pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &cloud) const
  {
    ...
    // std::cout<<"depthsrze"<<depth.rows<<" "<<depth.cols<<std::endl;
    // std::cout<<"colorsrze"<<color.rows<<" "<<color.cols<<std::endl;
